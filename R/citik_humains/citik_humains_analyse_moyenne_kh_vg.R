...
weatherHistogram <- function(paramDSK, paramMF, Breaks, paramName, SIunit){
        ...
        hist(paramDSK, breaks=Brx,
             ...
             ylab = "density",
             xlab = paste("Average",paramName,"(", SIunit, ")"),
...
templist <- list(dskParam=dskdatavg$temperature, mfParam=mfdatavg$temperature, Breaks=1, paramName='temperature', SIunit='C°')
humlist <- list(dskParam=dskdatavg$humidity, mfParam=trunc(mfdatavg$humidite), Breaks=1, paramName='humidity', SIunit='%')
ptrlist <- list(dskParam=dskdatavg$dewpoint, mfParam=mfdatavg$point_rose, Breaks=1, paramName='dew point', SIunit='C°')
presslist <- list(dskParam=dskdatavg$pressure, mfParam=mfdatavg$press_mer, Breaks=1, paramName='atmospheric pressure', SIunit='hPa')
vvlist <- list(dskParam=dskdatavg$windspeed, mfParam=mfdatavg$vvent, Breaks=0.5, paramName='wind speed', SIunit='m/s')
visiblist <- list(dskParam=dskdatavg$visibility, mfParam=mfdatavg$visibilite, Breaks=1, paramName='visibility', SIunit='%')
neblist <-list(dskParam=dskdatavg$cloudcover, mfParam=trunc(mfdatavg$nebulosite), Breaks=2, paramName='cloud cover', SIunit='%')
raflist <- list(dskParam=dskdatavg$windgust, mfParam=mfdatavg$rafale_10min, Breaks=0.5, paramName='wind gusts over 10 min', SIunit='m/s')
preciplist01 <- list(dskParam=dskdatavg$precipintensity, mfParam=mfdatavg$precip_01h, Breaks=0.1, paramName='rainfall intensity over 1 h', SIunit='%')
preciplist24 <- list(dskParam=dskdatavg$precipintensitymax, mfParam=mfdatavg$precip_24h, Breaks=0.1, paramName='maximum rainfall intensity over 24 h', SIunit='%')
