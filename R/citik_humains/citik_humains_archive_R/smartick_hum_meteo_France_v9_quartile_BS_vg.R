...
## 2.2 Création des dataframe par région comme comparées dans l'article
...
# humdata <- humdata[humdata$departement_code %in% c("75", "77", "78", "91":"95"), ]

# DSKdata <- DSKdata[DSKdata$departement_code %in% c("75", "77", "78", "91":"95") , ]



### 2.2.2 Création des subset pour l'AL

# humdata <- humdata[humdata$departement_code %in% c("54", "55", "57", "88", "67", "68"), ]

# DSKdata <- DSKdata[DSKdata$departement_code %in% c("54", "55", "57", "88", "67",  "68"), ]


### 2.2.3 Création des subset pour RA

# humdata <- humdata[humdata$departement_code %in% c("01", "07", "26", "38", "42", "69", "73", "74"), ]

# DSKdata <- DSKdata[DSKdata$departement_code %in% c("01", "07", "26", "38", "42", "69", "73", "74"), ]



# 3. Calcul des indicateurs avec leur intervalles de confiance par Boostrap
...
## 3.1 Scipt de la fonction Quartile


quartile <- function(param){

        c1 <- c(0:999)*NA ## création d'une table remplie de NA
        ...
        ###  Calcul des bornes à 0.05 et 0.95 des IC (intervalles de confiance)
        ...
        # print(output_Fr) ## affichage du résultat pour test
        return(output_Fr)
...
## 3.2 Sélection des paramètres (variables météo)

### 3.2.1 Sélection de quelques paramètres (ceux de l'article)

index <- c("temperature", "temperaturehigh", "humidity", "dewpoint", "pressure", "windspeed", "visibility", "cloudcover", "windgust", "uvindex")



### 3.2.2 Sélection de tous les paramètres (plus que pour l'article)

### sapply applique la fonction à droite au dataframe cible en retournant un vecteur booléen
# index <- sapply(humdata, is.numeric)


### Affichage des variables dans le vecteur booléen
index


## 3.3 Création de la table de résultats

### 3.3.1 Création d'une liste vide à remplir
ic_table <- list()
ic_table


### 3.3.2 Création d'une boucle de calculs implementant la fonction quartile

### avec filtrage par le vecteur index sur le dataframe humdata

for (name in names(humdata[,index])  ){

        # isolement de la colonne cible pour chaque iteration de la boucle
        param <- humdata[,name]
...
### /!\ Rajouter si nécessaire "_idf", "_AL", "_RA" à ic_quartiles.csv => par exemple "ic_quartiles_idf.csv"


## 4.1 Au format csv

write.csv(ic_table, 'ic_quartiles_signalements.csv')


## 4.1 Au format tabulé

write.table(ic_table,"results_quartile_signalements.txt", append = TRUE, sep="\t", col.names = TRUE, row.names = TRUE)
