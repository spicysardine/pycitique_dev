###### Code Tableau n°8 – Caractéristiques hivernales (octobre à mars), pour la France entière, selon le 1er décile, la moyenne et le 9ème décile des

###### paramètres météorologiques associés aux 1 095 lieux et dates de signalements comparés à ceux des mêmes dates
...
### /!\ pour tester les différences sur les hivers => subset hiver (W) /!\
...
humdata_tot <- read.csv("../../pycitique/data/donnee_signalements_avec_meteo_dsk/citik_humains_clean_weather_strict.csv", sep = ",", dec = ".", quote = "'", stringsAsFactors = FALSE)

DSKdata_tot <- read.csv("../../pycitique/data/donnee_meteo_nationale_comparative/darksky/darksky_donnee_brute_700_def3.csv", sep = ",", dec = ".", quote = "'", stringsAsFactors = FALSE)



## 2.0 Création des subsets

## transformation des chr en dates

library("lubridate")

humdata_tot$date_piq <- ymd(humdata_tot$date_piqure_saisie)

DSKdata_tot$date_piq <- ymd(DSKdata_tot$date_releve)


## /!\ Libérer la saison à traiter  /!\


### 2.0.1. Sélection de la période hivernale "longue" (6 mois)

humdata_W17 <- subset(humdata_tot, date_piq > "2017-10-01" & date_piq  < "2018-03-31")
humdata_W18 <- subset(humdata_tot, date_piq > "2018-10-01" & date_piq  < "2019-03-31")
humdata_W19 <- subset(humdata_tot, date_piq > "2019-10-01" & date_piq  < "2020-03-31")

humdata <-rbind(humdata_W17, humdata_W18, humdata_W19)


DSKdata_W17 <- subset(DSKdata_tot, date_piq > "2017-10-01" & date_piq  < "2018-03-31")
DSKdata_W18 <- subset(DSKdata_tot, date_piq > "2018-10-01" & date_piq  < "2019-03-31")
DSKdata_W19 <- subset(DSKdata_tot, date_piq > "2019-10-01" & date_piq  < "2020-03-31")

DSKdata <-rbind(DSKdata_W17, DSKdata_W18, DSKdata_W19)


### 2.0.2. Sélection de la période hivernale "courte" (4 mois)

humdata_W17 <- subset(humdata_tot, date_piq >= "2017-11-01" & date_piq  <= "2018-02-28")
humdata_W18 <- subset(humdata_tot, date_piq >= "2018-11-01" & date_piq  <= "2019-02-28")
humdata_W19 <- subset(humdata_tot, date_piq >= "2019-11-01" & date_piq  <= "2020-02-28")

humdata <-rbind(humdata_W17, humdata_W18, humdata_W19)


DSKdata_W17 <- subset(DSKdata_tot, date_piq >= "2017-11-01" & date_piq  <= "2018-02-28")
DSKdata_W18 <- subset(DSKdata_tot, date_piq >= "2018-11-01" & date_piq  <= "2019-02-28")
DSKdata_W19 <- subset(DSKdata_tot, date_piq >= "2019-11-01" & date_piq  <= "2020-02-28")

DSKdata <-rbind(DSKdata_W17, DSKdata_W18, DSKdata_W19)



### 2.0.3. Sélection de la période estivale
...
write.table(output_Fr,"results_winter_Fr.txt", append = TRUE, sep=";", col.names = TRUE, row.names = TRUE)
...
write.table(output_Fr,"results_winter_Fr.txt", append = TRUE, sep=";", row.names = TRUE)
...
## /!\ Renommer le output en fonction du subset traité  /!\
